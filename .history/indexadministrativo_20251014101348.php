...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Alerts para AULA y GRUPO solo cuando vienen de procesar-aula.php o procesar-grupo.php
    if(isset($_SESSION['msg_aula'])){
        echo "Swal.fire({icon:'success', title:'¡Éxito!', text:'Aula registrada correctamente', timer:2500, showConfirmButton:false});";
        unset($_SESSION['msg_aula']);
    }
    if(isset($_SESSION['error_aula'])){
        if($_SESSION['error_aula'] === 'repetido'){
            echo "Swal.fire({icon:'error', title:'Error', text:'Ya existe un aula con ese código', timer:2500, showConfirmButton:false});";
        } else {
            echo "Swal.fire({icon:'error', title:'Error', text:'Ocurrió un error al guardar el aula', timer:2500, showConfirmButton:false});";
        }
        unset($_SESSION['error_aula']);
    }

    if(isset($_SESSION['msg_grupo'])){
        echo "Swal.fire({icon:'success', title:'¡Éxito!', text:'Grupo registrado correctamente', timer:2500, showConfirmButton:false});";
        unset($_SESSION['msg_grupo']);
    }
    ?>
});
</script>
