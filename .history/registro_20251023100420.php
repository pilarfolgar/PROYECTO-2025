...
<head>
  ...
  <style>
    ...
    #password-strength { height: 8px; border-radius: 5px; background-color: #ddd; margin-top: 5px; overflow: hidden; }
    #password-strength-fill { height: 100%; width: 0%; border-radius: 5px; transition: width 0.3s; }

    /* Lista de requisitos */
    #password-requirements li { margin-bottom: 3px; transition: color 0.2s; font-size:0.85rem; }
    #password-requirements li.ok { color: green; }
    #password-requirements li.bad { color: red; }
  </style>
</head>
<body>
...
<main class="container my-5">
  <form action="procesar_registro.php" method="POST" id="registro_form">
    ...
    <div class="mb-3">
      ...
      <div class="form-text">Debe tener entre 12 y 16 caracteres, con mayúsculas, minúsculas, números y símbolos (@!$%&*?).</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Fuerza de contraseña y requisitos
  ...
  <?php if(!empty($_SESSION['registro_exitoso'])): ?>
    Swal.fire({icon:'success', title:'Registro exitoso', text:'¡Te registraste correctamente!', confirmButtonText:'OK'})
    .then((result) => { if(result.isConfirmed){ window.location.href='iniciosesion.php'; } });
  ...
  const campos = document.querySelectorAll('#registro_form input, #registro_form select');
