...
<section class="mis-cursos container mt-4">
  ...
  <div class="docentes-grid">
    ...
    // Consulta para traer los grupos del docente logueado
    $stmt = $con->prepare("
        SELECT DISTINCT g.id_grupo, g.nombre AS grupo_nombre, g.orientacion, g.cantidad_estudiantes
        FROM grupo g
        INNER JOIN grupo_asignatura ga ON g.id_grupo = ga.id_grupo
        INNER JOIN docente_asignatura da ON ga.id_asignatura = da.id_asignatura
    ...
    if ($result_grupos && $result_grupos->num_rows > 0) {
        while ($row = $result_grupos->fetch_assoc()) {
            ...
            echo '<div class="docente-name fw-bold">'.htmlspecialchars($row['grupo_nombre']).'</div>';
            echo '<div class="docente-subject text-muted">'.htmlspecialchars($row['orientacion']).'</div>';
            echo '<button class="btn btn-outline-primary boton ver-miembros" data-clase="'.htmlspecialchars($row['grupo_nombre']).'">Ver miembros</button>';
